...
our $VERSION = q{0.002};
use Moo;
use App::PerlGraph::Model qw(package_of qualify is_builtin);
...
sub _resolve_refs ($self) {
    ...
    for my $ref ($s->all_unresolved) {
        ...
        if (is_builtin($name)) { $self->_consume($ref->{id}); next }

        my ($target, $prov);
        if (($ref->{reference_kind} // '') eq 'method_call') {
...
sub _from_package ($self, $ref) {
    my $from = $ref->{from_node_id} ? $self->store->node($ref->{from_node_id}) : undef;
    return 'main' unless $from;
    # a package/class node IS the package; only sub nodes need the ::-strip.
    return $from->{qualified_name} // 'main' if ($from->{kind} // '') =~ /\A(?:package|class)\z/;
    return package_of($from->{qualified_name} // $from->{name} // 'main');
}
...
sub _resolve_call ($self, $ref) {
    ...
    my $name = $ref->{reference_name};
    if ($name =~ /::/) {
        my ($n) = grep { $_->{kind} =~ /function|method|constant/ } $s->nodes_by_qname($name);
        return $n;
    }
    my $pkg = $self->_from_package($ref);
    if (my ($n) = grep { $_->{kind} =~ /function|method|constant/ } $s->nodes_by_qname(qualify($pkg, $name))) {
        return $n;
    }
...
sub _resolve_method ($self, $ref) {
    ...
    my $recv = ($ref->{candidates} || {})->{receiver} // '';
    my ($start, $prov);
    if    ($recv =~ /\A\$(?:self|class)\z/) { $start = $self->_from_package($ref); $prov = 'heuristic' }
    elsif ($recv =~ /\A[\w:]+\z/)           { $start = $recv;                      $prov = 'static'    }
    else                                    { return }   # unknown receiver ($obj, expression, chain)
    return unless defined $start;

    for my $cls ($self->_mro($start)) {
...
Post-extraction pass linking unresolved refs to definitions (including static C<$self>/C<$class> resolution up the MRO) and filling inheritance edges by name.
