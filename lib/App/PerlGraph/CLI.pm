...
usage: pcg <command> [args] [path]
  index [--runtime] [--jobs N] [--max-file-size SZ]  build/refresh the graph
        (--jobs: parse workers; --max-file-size 1M: skip huge generated-data files)
  sync  [--runtime]     incremental update
  watch [--interval N] [--poll]  re-index on change (inotify on Linux, else poll; Ctrl-C to stop)
  ...
  affected [--tests] [--stdin] [--path DIR] <files>   files/tests impacted by a change
...
sub _cmd_index (@args) {
    ...
    my ($jobs, $maxsz, @rest);
    for (my @a = grep { $_ ne '--runtime' } @args; @a; ) {
        my $x = shift @a;
    ...
    my $stats = App::PerlGraph::Indexer->new(store => _store($root), root => $root,
        runtime => $runtime, ($jobs ? (jobs => $jobs) : ()), ($maxsz ? (max_file_size => $maxsz) : ()))->index_all;
    say "indexed $stats->{files} files ($stats->{reindexed} (re)parsed)" . ($runtime ? " + runtime enrichment" : "");
    return 0;
}
...
sub _cmd_watch (@args) {
    my ($interval, $once, $poll, $root) = (2, 0, 0, undef);
    while (@args) {
        my $a = shift @args;
    ...
    if ($once) { say "synced: ", $idx->sync->{reindexed}, " file(s) changed"; return 0 }
    ...
    while (1) {
        ...
        my $n = $idx->sync->{reindexed};
        say scalar(localtime) . " -- synced $n file(s)" if $n;
    }
}
...
sub _cmd_install (@) {
    ...
    say "Registered pcg with Claude Code (~/.claude.json). Restart Claude Code to load it.";
    return 0;
}

sub _cmd_uninstall (@) {
    App::PerlGraph::Installer->new->uninstall;
    say "Removed pcg from Claude Code config.";
    return 0;
}
...
unused), export, status, serve and install/uninstall.
