...
my @TOOLS = (
    ...
    { name => 'pcg_sinks', handler => 'sinks', description => 'Security attack surface: command-execution (system/exec/syscall) and SQL-execution (DBI do/execute/select*) call sites, and -- for a web app -- which routes/endpoints can transitively REACH each sink (route handler -> call closure). Each sink is flagged [dynamic] when its command/SQL STRING is built from a variable (interpolated or concatenated) -- the injection-shaped sites to verify; a constant or placeholdered call is parameterized and safe. Still heuristic (no full taint dataflow), so a [dynamic] reached sink is a site to VERIFY for tainted input, not a confirmed bug.',
      inputSchema => { type => 'object', properties => {} } },
    { name => 'pcg_taint', handler => 'taint', description => "Source -> sink TAINT PATHS: trace a call path from a user-input SOURCE to an injectable SINK. Sharper than pcg_sinks (route->sink reachability): the SOURCES are broader -- a web endpoint, any sub that calls a request accessor (param / params / cookie(s) / upload(s) / header(s) / query_parameters / body_parameters / route_parameters), OR any sub that reads external process input (\$ENV / \@ARGV / STDIN -- CLI/env injection) -- and only DYNAMIC sinks (whose command/SQL string is built from a variable -- the injectable ones) are targets, with the actual call PATH shown (source -> ... -> sink). CONFIDENCE tiers: a `[value-flow]` hit applies INTRA-PROCEDURAL value-flow -- the tainted value provably reaches the sink's ARGUMENT inside one sub (the strongest signal); a `[cross-sub]` hit is a VERIFIED inter-procedural flow -- the source's value propagates argument->parameter through the call chain into the sink's argument (a confirmed cross-sub path, not mere reachability); a plain `[local]` hit is co-located (source + sink share the sub, but the value may not reach the arg); an untagged path is call-graph REACHABILITY only. So `[value-flow]` (intra-sub) and `[cross-sub]` (inter-procedural) are both CONFIRMED value-flow; `[local]`/untagged are the leads to VERIFY.",
      inputSchema => { type => 'object', properties => {
